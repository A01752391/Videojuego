...
export function renderBoard(gameContext) {
    console.log('renderBoard called with gameContext:', gameContext);
    ...
    const kingIsCurrentlyInCheck = gameContext.isKingInCheck && gameContext.isKingInCheck(board, currentColor, gameContext);

    let possibleMovesList = [];
    if (selected && gameContext.getPossibleMoves) {
    ...
    for (let r = 0; r < 8; r++) {
        for (let c = 0; c < 8; c++) {
            ...
            console.log(`Piece at [${r}][${c}]:`, piece);
            ...
            if (piece) {
                ...
                console.log(`Adding image class ${imageClass} for piece:`, piece);
